...
DECLARE
    ...
    -- Cache per performance: ID lingue e feature MAG_RIF (valorizzati una volta all'avvio)
    w_language_ids INTEGER[];
    w_mag_rif_feature_id INTEGER;
...
BEGIN
    -- Performance: cache w_language_ids e w_mag_rif_feature_id una volta; indice su temp_transaction_items(item);
    -- sostituite subquery ripetute con variabili (W_ITEM_SALE_ID, W_ITEM_LOGISTIC_ID).
    -- Suggerimento DBA: indici su tin_data_item_in(processing_step3, processing_step2, processing_step1, item)
    -- e sulle FK/join usate dal cursore cur_items per accelerare il cursore.
    --
    ...
    CREATE INDEX idx_temp_transaction_items_item ON temp_transaction_items (item);

    -- Cache una tantum: ID lingue e feature MAG_RIF (evita subquery ripetute nel loop)
    SELECT ARRAY_AGG(id ORDER BY id) INTO w_language_ids FROM tpa_languages;
    SELECT id INTO w_mag_rif_feature_id FROM tpa_specific_features WHERE specific_feature = 'MAG_RIF' LIMIT 1;
...
/* 					IF w_esito_processing_item = 1 THEN
	        	    ...
	        	    IF r_item_data.processing_item = 0 THEN
                        ...
                        IF r_item_data.update_item = 1 THEN
                            ...
                            SELECT EXISTS (SELECT 1 FROM ttr_items WHERE item_id = w_item_id AND language_id = ANY(w_language_ids) FOR UPDATE) INTO v_ttr_item_exists;
                            ...
                            IF v_ttr_item_exists THEN
                                ...
                                SET short_description = r_item_data.description_item,
                                    ...
                                    last_user = p_user,
                                    transaction_code = r_item_data.transaction_code
                                WHERE item_id = w_item_id AND language_id = ANY(w_language_ids);
                            ELSE
                        ...
                        IF r_item_data.update_sale = 1 THEN
                            ...
                            SELECT EXISTS (SELECT 1 FROM ttr_item_sales WHERE item_sale_id = W_ITEM_SALE_ID AND language_id = ANY(w_language_ids) FOR UPDATE) INTO v_ttr_item_sale_exists;
                            ...
                            IF v_ttr_item_sale_exists THEN
                                ...
                                SET description = r_item_data.description_sale,
                                    last_user = p_user,
                                    transaction_code = r_item_data.transaction_code
                                WHERE item_sale_id = W_ITEM_SALE_ID AND language_id = ANY(w_language_ids);
                            ELSE
...
--                            END IF;
                           ...
                           -- Inserimento del codice a barre '7999' se non esiste e Mag_Rif = 'R99'
                            ...
                            AND EXISTS (
                                ...
                                AND specific_feature_id = w_mag_rif_feature_id
